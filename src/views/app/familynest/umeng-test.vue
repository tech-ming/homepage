...
let ULink = null;
let data = {};
const linkid_default= "usr1h49s0l7ngbh5"
let ctx = null;
...
function loadScripts() {
  return new Promise((resolve) => {
    ...
    ulinkScript.src =
      "https://share.umeng.com/demo/ulink/index.min.js?v=${SDK_VERSION}";
    ulinkScript.onload = () => {
      ULink = window.ULink;
...
function proxyOpenDownloadfn(defaultAction, cctx) {
  if (data.popup === "true") {
    ...
    } else if (cctx.solution.type === "universalLink") {
...
function initOldULink(tm, tipitem, auto, clp) {
  setTimeout(() => {
    ...
    ULink.tracker.enter(
      { page_name: document.title, page: location.href },
      () => {
